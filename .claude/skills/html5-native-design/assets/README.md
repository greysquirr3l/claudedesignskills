...
`groundtruth/html5_apis_lab.html` — a copy of the user's
`2026_html5_apis.html` from
[greysquirr3l/pattern_lab](https://github.com/greysquirr3l/pattern_lab).
Thirteen finished Web-platform-API examples (LocalStorage,
Geolocation, DnD, Web Workers, IntersectionObserver, Clipboard,
Fullscreen, History, Web Speech, Notifications, Page Visibility,
BroadcastChannel, Popover) with the dark navy / neon palette.

`groundtruth/periodic_table.html` — a copy of the
`periodic_table.html` from `greysquirr3l/pattern_lab`. A complete
interactive periodic table built with native HTML (popovers,
container queries, no framework) — useful as a finished-design
reference.
